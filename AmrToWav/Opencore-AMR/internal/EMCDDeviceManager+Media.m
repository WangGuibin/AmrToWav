...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if (![fileManager fileExistsAtPath:wavFilePath]) {
        BOOL covertRet = [self convertAMR:aFilePath toWAV:wavFilePath sampleRate: Sample_Rate_16000];
        if (!covertRet) {
            if (completon) {
    ...
    [EMAudioRecorderUtil asyncStopRecordingWithCompletion:^(NSString *recordPath) {
        if (completion) {
            if (recordPath) {
                ...
                BOOL convertResult = [self convertWAV:recordPath toAMR:amrFilePath sampleRate: Sample_Rate_16000];
                if (convertResult) {
                    // 删除录的wav
                    NSFileManager *fm = [NSFileManager defaultManager];
...
- (BOOL)convertAMR:(NSString *)amrFilePath
    ...
    if (isFileExists) {
        //因为网上大都是8000采样率的 现在这个项目用的是16000的所以先这样设置 后续有其他改变再开放设置采样率优化吧
        BOOL success = [VoiceConverter DecodeAmrToWav: amrFilePath wavSavePath: wavFilePath sampleRateType:rate];
        NSLog(@"%@",success? @"解码成功":@"解码失败");
...
- (BOOL)convertWAV:(NSString *)wavFilePath
    ...
    if (isFileExists) {
        BOOL success = [VoiceConverter EncodeWavToAmr: wavFilePath amrSavePath: amrFilePath sampleRateType: rate];
        NSLog(@"%@",success? @"编码成功":@"编码失败");
